...
def build_rag_chain(config_path="config.yaml"):
    ...
    llm = build_object(get_component_from_path(config, query_pipeline_config["llm"]))
    embeddings = build_object(get_component_from_path(config, config["pipeline"]["ingestion"]["embedder"]))
    print("Loading vector store...")
    db = FAISS.load_local(config["vector_store_path"], embeddings, allow_dangerous_deserialization=True)
    base_retriever = db.as_retriever(**get_component_from_path(config, query_pipeline_config["retriever"]))
    final_retriever = base_retriever
    if "reranker" in query_pipeline_config:
        print("Building re-ranker...")
        reranker_config = get_component_from_path(config, query_pipeline_config["reranker"])
